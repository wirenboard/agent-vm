...
- ВМ управляются через Lima/QEMU (macOS/Linux) или WSL2 (Windows)
- Хост ↔ ВМ связь: `host.lima.internal` (Lima) или `localhost` (WSL2 — оба дистрибутива в одном сетевом пространстве Hyper-V)
...
```
┌─────────────────────────────────────────────────────────────────┐
│ Хост (WSL2 host distro)                                         │
│                                                                 │
│  Токены ──► credential-proxy.py ──────► api.anthropic.com       │
│             (127.0.0.1:PORT)            github.com              │
│                  ▲                      api.github.com          │
│                  │ HTTP (localhost:PORT — shared Hyper-V netns) │
│ ─ ─ ─ ─ ─- ─ ─ ─ │ -----─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ │
│ Agent WSL2 distro│                                              │
│ (DrvFs disabled) │                                              │
│  Claude ──► mitmdump ────────────► pypi.org, npm и др.          │
...
│  gh                                                             │
│                                                                 │
│  Файловая система: только /mnt/wsl/ bind-маунты                 │
│  (project dir + state dir; C: недоступен)                       │
└─────────────────────────────────────────────────────────────────┘
```
...
**Запуск:**
- Lima: через `systemd-run --user` — процесс переживает завершение сессии `limactl shell`.
- WSL2: через `nohup setsid ... &` — `systemd-run --user` ненадёжен в эфемерных инстансах без полноценной systemd user-сессии.
...
### ADR-5: WSL2 — отключение DrvFs автомонтирования

WSL2 по умолчанию автоматически монтирует все Windows-диски (`/mnt/c`, `/mnt/d`, ...) в каждом дистрибутиве. Без ограничений агент получает полный доступ к файловой системе Windows.

**Решение:** В `wsl.conf` шаблона устанавливается `[automount] enabled=false`. Агентские дистрибутивы загружаются без DrvFs. Нужные директории (проект, state dir) монтируются явно через `/mnt/wsl/` bind-маунты из host-дистрибутива (у которого DrvFs всегда включён). Внутри агента создаётся симлинк с ожидаемого пути на `/mnt/wsl/`-путь.

| | DrvFs включён (было) | DrvFs отключён (стало) |
|---|---|---|
| Доступ к C: | Полный | Нет |
| Доступ к проекту | Через `/mnt/c/...` | Через bind-маунт `/mnt/wsl/` |
| SSH-ключи, браузер | Доступны агенту | Недоступны |
| Изоляция | Хуже Docker | На уровне Lima |

**Побочные изменения:**
- `_wsl_run_script` переведён с Windows temp (`AppData/Local/Temp`) на `/mnt/wsl/agent-vm-tmp/` — скрипты передаются через shared tmpfs, без DrvFs.
- `_wsl_mount_dir` убран early-return для Windows-путей — все пути проходят через bind-маунт.

### ADR-6: Отказ от retry-логики
...
### ADR-7: Per-instance секрет для изоляции ВМ
