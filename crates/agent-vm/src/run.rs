...
/// Best-effort hint at where msb writes a sandbox's `runtime.log`.
/// Used solely in the `with_context` of attach errors, so a wrong
/// guess just dilutes the hint rather than breaking anything. Matches
/// upstream's `microsandbox_utils::resolve_home()` fallback order
/// (`$MSB_HOME` → `$HOME/.microsandbox` → `./.microsandbox`).
///
/// Adjacent files in the same directory (`msb.stderr.log`,
/// `msb-exit.log`) carry libkrun stderr and per-boot exit status
/// respectively — see vendor `runtime::spawn` and `runtime::handle`.
/// `runtime.log` is the most useful pointer for the common
/// in-VM-tracing-but-attach-failed case, which is why this hint
